...
function decorate<T extends Decoratable>(listing: T, favouriteIds: Set<string> = new Set()) {
...
router.get('/', optionalAuth, async (req, res, next) => {
  try {
    ...
    const orderByOptions: Record<typeof q.sort, Prisma.ListingOrderByWithRelationInput> = {
    ...
    const [rows, total] = await Promise.all([
      prisma.listing.findMany({ where: { AND }, include: cardShape, orderBy, take: q.take, skip: q.skip }),
      prisma.listing.count({ where: { AND } }),
    ]);

    let favs = new Set<string>();
    if (req.userId) {
      const f = await prisma.favorite.findMany({ where: { userId: req.userId }, select: { listingId: true } });
  ...
  try {
    const listing = await prisma.listing.findUnique({
      ...
      include: {
        ...
        category: true,
        host: { select: { id: true, name: true, avatarUrl: true, hostSince: true, bio: true } },
        amenities: { include: { amenity: true } },
        reviews: {
          orderBy: { createdAt: 'desc' },
          include: { user: { select: { name: true, avatarUrl: true, createdAt: true } } },
        },
      },
    });
    if (!listing) { res.status(404).json({ error: 'This place is no longer listed' }); return; }

    type ScoreKey = 'overall' | 'cleanliness' | 'accuracy' | 'checkin' | 'communication' | 'location' | 'value';
    const r = listing.reviews;
    const avgOf = (k: ScoreKey) => (r.length ? Number((r.reduce((a, x) => a + x[k], 0) / r.length).toFixed(2)) : null);
    const breakdown = {
  ...
  try {
    ...
    const [bookings, blocked] = await Promise.all([
      prisma.booking.findMany({
        where: { listingId: req.params.id, status: 'CONFIRMED', checkOut: { gt: from }, checkIn: { lt: to } },
    ...
    for (const b of bookings) {
      ...
      for (const d of eachDayOfInterval({ start: b.checkIn, end: new Date(b.checkOut.getTime() - 86400000) })) {
...
router.post('/:id/quote', async (req, res, next) => {
  try {
    const { checkIn, checkOut } = z.object({ checkIn: z.string(), checkOut: z.string() }).parse(req.body);
    const listing = await prisma.listing.findUnique({ where: { id: req.params.id } });
    if (!listing) { res.status(404).json({ error: 'This place is no longer listed' }); return; }
    res.json({ currency: listing.currency, pricePerNight: listing.pricePerNight, ...quote(listing, checkIn, checkOut) });
  } catch (e) { next(e); }
});
